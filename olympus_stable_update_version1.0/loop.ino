...
void loop()
{
  ...
  else if (ft9  == 1)
  {
    ...
    {
      digitalWrite(s9 , proff);
      Serial.println("PROJECTOR SWITCH OFF");
      f9  = proff; a9  = 0; x9  = 0; ft9  = 0;
      prflag = temp;
      pf();
    }
  }
  ...
  // special fan
 else if (waitflag == 1 && flowflag == 0) //waiting so turn off
  {
    digitalWrite(s3, 0);
    if (millis() > (waitstt + waitdur))
    {
      waitflag = 0;
      flowflag = 1; //waiting over
      starter();
    }
  }

  else if (flowflag == 1 && waitflag == 0)
  {
    digitalWrite(s3, 1);
    if (millis() > (flowstt + flowdur))
    {
      waitflag = 1;
      flowflag = 0; // start waiting
      starter();
    }
  }
