...
int ledAnalogTwo[] = {9, 10, 11};
                                      //the three pins of the second analog LED 9 = redPin, 10 = greenPin, 11 = bluePin
                                      //These pins must be PWM
...
void loop()
{
  /* Example 1 - Defined Colors    Set to a known color (you can use any of the above defined colors)*/
  setColor(ledAnalogOne, MAGENTA);
  /* Example 2 - Any Color    Set the LED to any color you like*/
                                        //byte tempColor[] = {12,34,12};
                                        //the RGB (red, gren blue) value for a color to display
                                        //setColor(ledAnalogOne, tempColor);
  /*Example 3 - Fading  Fade the LED between two colors (this will go from red to green to blue then back to red)*/
...
/* Sets the color of the LED to any RGB Value
  ...
  (color[0] = new Red value,
  color[1] = new Green value,
  color[2] = new Red value*/
void setColor
(int* led, byte* color)
{
  for (int i = 0; i < 3; i++)
...
/* A version of fadeToColor that takes predefined colors
  (neccesary to allow const int pre-defined colors */
void fadeToColor
(int* led, const byte* startColor, const byte* endColor, int fadeSpeed)
{
